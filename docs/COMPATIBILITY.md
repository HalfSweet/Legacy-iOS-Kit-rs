...
| iPhone X restored_external | ✅ ⚠️ | `lik firmware ipx-prepare` builds `kcache.im4p` (KPlooshFinder AMFI patch set + APFS snapshot rename, LZSS `rkrn` repack) and `rdsk.im4p` (restored_external FaceID patch, ad-hoc re-sign with the original entitlements, HFS+ swap, uncompressed `rdsk` repack) from a stock 14.3-15.x IPSW; `lik restore plan/execute --rdsk --rkrn` feed them into the restore boot chain and force RSEP send; A11 entry goes through checkm8 pwnDFU (upstream's forced usepwndfu64); hardware-unverified (KPlooshFinder hit rate against real kernelcaches, restored_external patch + entitlements re-sign, rdsk/rkrn override boot) |
